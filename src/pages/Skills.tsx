          ...
          <div key={c.category} className="bg-white dark:bg-gray-800 dark:text-gray-100 rounded-md shadow-sm overflow-hidden border border-transparent dark:border-gray-700">
            ...
            <div className="p-6">
              <ul className="space-y-4">
                {(() => {
                  const visibleItems = expandedCategory[c.category] ? c.items : c.items.slice(0, 3)
              ...
              {c.items.length > 3 && (
                <div className="mt-3">
                  <button
                    onClick={() => setExpandedCategory((prev) => ({ ...prev, [c.category]: !prev[c.category] }))}
                    className="text-sm text-blue-600 hover:underline"
                    aria-expanded={!!expandedCategory[c.category]}
                  >
                    {expandedCategory[c.category] ? 'View less' : `View more (${c.items.length - 3})`}
                  </button>
                </div>
              )}
            </div>
